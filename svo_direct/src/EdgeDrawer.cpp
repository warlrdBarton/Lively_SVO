...
void EdgeDrawer::init(const LineDetectionExtraInfoPtr &gradientInfo, cv::Mat &edgeImage) {
  assert(gradientInfo->imageWidth == static_cast<unsigned int> (gradientInfo->gImg.cols));
  assert(gradientInfo->imageHeight ==static_cast<unsigned int> ( gradientInfo->gImg.rows));
...
inline void EdgeDrawer::addJunctionPixelsToSegment(const ImageEdge &junctionPixels,
  ...
  for (size_t i = 0; i < junctionPixels.size(); i++) {
...
//         << "\n\t- Initial Pixels: " << branch.pixels;
    ...
    while (outliersList.size() <= UPM_MAX_OUTLIERS_TH) {
      ...
      if (segment) {
        ...
        if (static_cast<int>(pixels.size()) - lastChekedPxIdx >= minLineLength) {
...
Segments EdgeDrawer::getDetectedSegments() const {
  ...
  for (size_t i = 0; i < segments.size(); i++) result[i] = segments[i].getEndpoints();
  return result;
}
