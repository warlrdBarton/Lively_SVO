#include <vector>
#include "svo/direct/FullSegmentInfo.h"
...
void FullSegmentInfo::init(const std::vector<Pixel> &pts, int startIdx) {
  assert(static_cast<int>(pts.size()) > startIdx);
  assert(pts.size() >= 2);
...
void FullSegmentInfo::skipPositions() {
  ...
  assert(static_cast<int>(pixels->size()) == lastPxIndex + 1 + UPM_SKIP_EDGE_PT);
