...
PointFeature::PointFeature( Vector2d pl_, double disp_, Vector3d P_, int idx_, int level_, Matrix3d covP_an_ ) :
     P(P_),pl(pl_), disp(disp_), inlier(true), idx(idx_), level(level_), covP_an(covP_an_)
{
    for( int i = 0; i < level; i++ )
        sigma2 *= Config::orbScaleFactor();
    sigma2 = 1.f / (sigma2*sigma2);
...
PointFeature::PointFeature( Vector2d pl_, double disp_, Vector3d P_, Vector2d pl_obs_ ) :
     P(P_),pl(pl_), pl_obs(pl_obs_), disp(disp_), inlier(true), level(0)
{}

PointFeature::PointFeature( Vector2d pl_, double disp_, Vector3d P_, Vector2d pl_obs_,
              int idx_, int level_, double sigma2_, Matrix3d covP_an_, bool inlier_ ) :
    P(P_),pl(pl_),   pl_obs(pl_obs_), disp(disp_),inlier(inlier_), level(level_), sigma2(sigma2_), covP_an(covP_an_)
{}
...
LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
     ...
     sdisp(sdisp_),edisp(edisp_),sP(sP_),eP(eP_),  le(le_), spl(spl_), epl(epl_),  inlier(true), level(0)
{}

LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
                          Vector2d epl_, double edisp_, Vector3d eP_,
                          Vector3d le_, Vector3d le_obs_ ) :
   sdisp(sdisp_), edisp(edisp_), sP(sP_),  eP(eP_), le(le_), le_obs(le_obs_), spl(spl_), epl(epl_), inlier(true), level(0)
{}

LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
                          Vector2d epl_, double edisp_, Vector3d eP_,
                          Vector3d le_,  int    idx_) :
    idx(idx_),sdisp(sdisp_),edisp(edisp_), sP(sP_), eP(eP_),  le(le_), spl(spl_), epl(epl_), inlier(true),  level(0)
{}

LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
                          Vector2d epl_, double edisp_, Vector3d eP_,
                          Vector3d le_,  double angle_, int    idx_) :
    idx(idx_), sdisp(sdisp_), edisp(edisp_), angle(angle_),sP(sP_), eP(eP_), le(le_), spl(spl_),epl(epl_),  inlier(true),  level(0)
{}

LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_,
                          Vector2d epl_, double edisp_, Vector3d eP_,
                          Vector3d le_,  double angle_, int idx_, int level_) :
    idx(idx_),sdisp(sdisp_), edisp(edisp_),  angle(angle_), sP(sP_),  eP(eP_), le(le_),spl(spl_), epl(epl_), inlier(true),level(level_)
{
    for( int i = 0; i < level; i++ )
...
LineFeature::LineFeature( Vector2d spl_, double sdisp_, Vector3d sP_, Vector2d spl_obs_, double sdisp_obs_,
    ...
    idx(idx_),sdisp(sdisp_),edisp(edisp_),  angle(angle_),sdisp_obs(sdisp_obs_),edisp_obs(edisp_obs_),
    sP(sP_), eP(eP_), le(le_), le_obs(le_obs_),spl(spl_), epl(epl_),  spl_obs(spl_obs_), epl_obs(epl_obs_), 
       inlier(inlier_), level(level_),sigma2(sigma2_), covE_an(covE_an_), covS_an(covS_an_)
{
    for( int i = 0; i < level; i++ )
        sigma2 *= Config::lsdScale();
    sigma2 = 1.f / (sigma2*sigma2);
